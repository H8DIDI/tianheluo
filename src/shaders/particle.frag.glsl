...
void main() {
  ...
  float alpha = sprite.a * mix(glow, core, 0.78);
  ...
  float sparkle = 0.92 + 0.28 * fract(sin(vAge * 127.1 + gl_PointCoord.x * 311.7) * 43758.5453);
  alpha *= sparkle;

  float flash = smoothstep(0.82, 1.0, vLife);
  float boost = mix(uCoreBoost, uFlashBoost, flash);
  vec3 finalColor = vColor * sprite.rgb * (1.0 + core * boost + glow * 0.7);
